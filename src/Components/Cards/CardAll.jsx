...
const CardAll = () => {
    ...
    useEffect(() => {
      axios.get('https://wishlists-api-annelund.vercel.app/allmembers')
        .then((res) => {
          setData(res.data)
        })
    }, [])
    ...
    <>
      {isLoading ? <Loading/> : null} 
        {data?.map(wish => {
          ...
          return(
            <StyledCard key={wish.id} style={role_id < 3 && wish.købt === 1 ? {'opacity': '30%'} : null}>
                  <picture>
...
<div style={role_id > 2 ? {'display': 'none'} : {'display': 'block'}}> 
{wish.købt === 1 ? 
                    ...
                    <div>
                      ...
                      <button onClick={() => {
                        ...
                        const payload = {id: wish.id,købt: 1};
                         axios.put(`https://wishlists-api-annelund.vercel.app/allmembers`, payload)
                          .then((res) => {
                            console.log(res)
                              
                            if(res.data.message === "Product Updated") {
                            setFlashMessage('Tak - den bliver vi glade for!')
                            setIsLoading(false)
...
const payload = {
  ...
  axios.delete(`https://wishlists-api-annelund.vercel.app/allmembers`, payload)
  .then((res) => {
 console.log(res)
